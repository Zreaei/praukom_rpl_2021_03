...
<div class="">
    <div class="fixed w-20 mt-5 overflow-y-auto bg-gray-50 rounded-r-lg bg-gradient-to-b from-[#2D5EBB] to-[#417EF2]">
        <a href="{{ route('verifikator.home') }}" class="flex p-2 text-xl font-bold text-[#ffffff] rounded-r-lg hover:bg-[#ffffff] hover:bg-opacity-30  active:bg-opacity-30 focus:bg-[#ffffff] focus:bg-opacity-30">
    ...
    <div class="flex justify-center">
        <div class="px-28 py-7 w-11/12 justify-center items-center ">
            <div class="w-full justify-center items-center">
                <div class="flex justify-between">
                    <h3 class="text-2xl font-bold text-[#2D5EBB] mx-auto">DATA VERIFIKASI</h3>
                </div>
            <div>
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-5">
                <table class="w-full text-sm text-left text-[#ffffff]">
                    ...
                    <tbody>
                    @foreach ($dataVerifikasi as $item)
                        <tr class="bg-[#ffffff] text-[#000000]">
                            <td class="text-sm text-item font-semibold px-5 py-4 text-center">
                                <label for="detail-siswa{{ $item->siswa }}" type="button" class="text-[#ffffff] bg-[#0029FF] hover:bg-opacity-50 focus:outline-none focus:ring-4  font-medium rounded-lg text-sm px-5 py-2.5">Data Siswa</label>          
                            </td>
                            ...
                            <td class="text-sm font-medium leading-5 text-center [#ffffff]space-no-wrap ">
                                <div class="flex justify-center">
                                    <div class="px-4 py-4">
                                        <label for="konfirmasi{{ $item->id_verifikasi }}" class="text-[#2D5EBB] hover:text-opacity-50">
        ...
        @foreach ($dataVerifikasi as $konfirmasi)
            ...
            <div class="modal">
                <div class="modal-box w-full max-w-xl bg-[#2D5EBB]">
                    <label for="konfirmasi{{$konfirmasi->id_verifikasi}}" class="btn btn-ghost btn-sm btn-circle text-[#ffffff] bg-[#2D5EBB] hover:bg-[#ffffff] hover:text-[#2D5EBB] absolute right-2 top-2"><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></label>
                    <h3 class="text-lg font-bold text-[#ffffff] text-center">KONFIRMASI VERIFIKATOR</h3>

                    {{-- Walas --}}
                    <div class="form-control w-full max-w-sm mx-auto pt-8">
                        @switch($konfirmasi->konfirmasi_verifikator)
                            @case('terima')
                    ...
                    {{-- Konfirmasi Walas --}}
                    @if($konfirmasi->konfirmasi_verifikator == "pending")
