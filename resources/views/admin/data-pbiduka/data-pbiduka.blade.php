...
<div class="px-28 py-7 w-full justify-center items-center">
    <div class="flex justify-between">
        <h3 class="text-2xl font-bold text-[#2D5EBB] mx-auto">DATA PEMBIMBING IDUKA</h3>
    </div>
<div>
...
<div class="w-2/3 rounded mt-5 border-slate-500 mx-auto">
    <div class="overflow-x-auto">
        ...
        <thead>
            <tr>
                <th class="text-center bg-blue-700 text-white">nik</th>
        ...
        @foreach ($dataPbiduka as $item)
        <tbody>
            <!-- row 1 -->
            <tr>
                <td class="text-center bg-white"><h1>{{ $item->nik }}</h1></td>
                <td class="text-center bg-white"><h1>{{ $item->user }}</h1></td>
                <td class="text-center bg-white"><h1>{{ $item->nama_pbiduka }}</h1></td>
                <td class="text-center bg-white"><h1>{{ $item->telp_pbiduka }}</h1></td>
                <td class="text-center bg-white">
                    <label for="modal-edit{{ $item->nik }}" class="btn btn-circle btn-outline btn-xl btn-warning hover:text-opacity-50">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    </label>
                    <label for="modal-hapus{{ $item->nik }}" class="btn btn-circle btn-outline btn-xl btn-error hover:text-opacity-50 justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="m-3 text-center">
        {{-- Modal Button --}}
        <label for="modal-tambah" class="btn btn-success hover:text-opacity-50">Tambah Pbiduka</label>
        ...
        <div class="modal">
            <div class="modal-box relative bg-[#2D5EBB]">
                ...
                <h3 class="text-lg font-bold text-white">Tambah Pbiduka</h3>
                <p class="py-4">
                    <form method="POST" action="data-pbiduka/simpan">
                        @csrf
                        <div class="form-control">
                            <div class="mx-auto">
                                <label class="input-group m-5">
                                    <span class="pr-14 bg-white">NIK</span>
                                    <input type="number" name="nik" placeholder="NIK" class="input input-bordered" />
                                </label>
                                <label class="input-group m-5">
                                    <span class="pr-10 bg-white">Nama Pbiduka</span>
                                    <input type="text" name="nama_pbiduka" placeholder="Nama Pbiduka" class="input input-bordered" />
                                </label>
                                <label class="input-group m-5">
                                    <span class="pr-10 bg-white">No Telepon</span>
                                    <input type="number" name="telp_pbiduka" placeholder="No Telepon" class="input input-bordered" />
                                </label>
    ...
    <div class="text-center">
        @foreach ($dataPbiduka as $edit)
        <input type="checkbox" id="modal-edit{{ $edit->nik }}" class="modal-toggle" />
        <div class="modal">
            <div class="modal-box relative bg-[#2D5EBB]">
                <label for="modal-edit{{ $edit->nik }}" class="btn btn-ghost btn-xl btn-circle text-[#ffffff] bg-[#2D5EBB] hover:bg-[#ffffff] hover:text-[#2D5EBB] absolute right-2 top-2">✕</label>
                <h3 class="text-lg font-bold text-center text-white">Edit Pbiduka</h3>
                <p class="py-4">
                    <form method="POST" action="data-pbiduka/simpanedit">
                        @csrf
                        <div class="form-control">
                            <div class="mx-auto">
                                <label class="input-group m-5">
                                    <span class="pr-9 bg-white">NIK</span>
                                    <input type="number" name="nik" placeholder="NIK Pbiduka" value="{{ old('nik', $edit->nik) }}" class="input input-bordered"/>
                                </label>
                                <label class="input-group m-5">
                                    <span class="pr-5 bg-white">Nama Pbiduka</span>
                                    <input type="text" name="nama_pbiduka" placeholder="nama_pbiduka" value="{{ old('nama_pbiduka', $edit->nama_pbiduka) }}" class="input input-bordered" />
                                </label>
                                <label class="input-group m-5">
                                    <span class="pr-10 bg-white">No Telepon</span>
                                    <input type="number" name="telp_pbiduka" placeholder="No Telepon" value="{{ old('telp_pbiduka', $edit->telp_pbiduka) }}" class="input input-bordered" />
                                </label>
                                <label class="input-group justify-center">
    ...
    {{-- Modal Hapus --}}
    <div>
        @foreach($dataPbiduka as $hapus)
        <input type="checkbox" id="modal-hapus{{ $hapus->nik }}" class="modal-toggle" />
        <div class="modal">
            <div class="modal-box w-11/12 max-w-xl bg-white">
                ...
                <div class="flex justify-center pt-4 gap-x-20 gap-y-3">
                    <label for="modal-hapus{{ $hapus->nik }}" class="btn btn-ghost btn-base bg-[#2D5EBB] w-28 text-white text-base hover:bg-[#2D5EBB] hover:bg-opacity-70">Cancel</label>
                    <label class="btn btn-ghost btn-base bg-[#E63946] w-28 text-white text-base hover:bg-[#E63946] hover:bg-opacity-70">
                        <a href="data-pbiduka/hapus/{{ $hapus->nik }}">
                            Delete
                        </a>
                    </label>
                </div>
