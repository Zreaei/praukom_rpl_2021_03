...
<div class="grid place-items-center h-screen">
                <div class="py-20 px-20 w-11/12 max-w-2xl bg-[#2D5EBB]">
                    <h3 class="text-lg font-bold text-[#ffffff] text-center">EDIT DATA SISWA</h3>
                
                    <form action="/operator/editsimpansiswa" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                        <input type="hidden"  name="nis" value="{{ $edit[0]->nis }}" />
                        ...
                        <div class="flex justify-between mt-2">
                            <div class="form-control w-full max-w-sm mr-2">
                                ...
                                <input type="text" name="nama_siswa" value="{{ $edit[0]->nama_siswa }}" class="input input-bordered w-full max-w-sm bg-[#ffffff]" required/>
                            </div>
                            <div class="form-control w-full max-w-sm">
                                ...
                                <select class="select select-bordered bg-white" name="kelas" required>
                                    ...
                                    <option value="{{ $item->id_kelas }}">{{ $item->tingkatan }} {{ $item->program_keahlian }} {{ $item->nama_kelas }} ({{ $item->tahun_angkatan }})</option>
                                    @endforeach
                                </select>  
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between mt-2">
                            <div class="form-control w-full max-w-sm mr-2">
                                ...
                                <input type="text" name="tempat_lahir" value="{{ $edit[0]->tempat_lahir }}" class="input input-bordered w-full max-w-sm bg-[#ffffff]" required/>
                            </div>
                            <div class="form-control w-full max-w-sm">
                                <label class="label"><span class="label-text text-[#ffffff] text-lg font-bold">TANGGAL LAHIR</span></label>
                                <input type="date" name="tgl_lahir" value="{{ $edit[0]->tgl_lahir }}" class="input input-bordered w-full max-w-sm bg-[#ffffff]" required/>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between mt-2">
                            ...
                            <div class="form-control w-full max-w-sm">
                                ...
                                <input type="number" name="telp_siswa" value="{{ $edit[0]->telp_siswa }}" class="input input-bordered w-full max-w-sm bg-[#ffffff]" required/>
                            </div>
                        </div>
