...
<div class="fixed w-20 mt-5 overflow-y-auto bg-gray-50 rounded-r-lg bg-gradient-to-b from-[#2D5EBB] to-[#417EF2]">
    <a href="{{ route('operator.verifikator') }}" class="flex p-2 text-xl font-bold text-[#ffffff] rounded-r-lg hover:bg-[#ffffff] hover:bg-opacity-30  active:bg-opacity-30 focus:bg-[#ffffff] focus:bg-opacity-30">
...
<div class="grid place-items-center h-screen">
                <div class="py-20 px-20 w-11/12 max-w-2xl bg-[#2D5EBB]">
                    <h3 class="text-lg font-bold text-[#ffffff] text-center">EDIT DATA VERIFIKATOR</h3>
                
                    <form action="/operator/editsimpanverifikator" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                        <input type="hidden"  name="id_verifikator" value="{{ $edit[0]->id_verifikator }}" />
                        ...
                        <div class="flex justify-between mt-2">
                            <div class="form-control w-full max-w-2xl">
                                ...
                                <input type="text" name="nip_verifikator" value="{{ $edit[0]->nip_verifikator }}" class="input input-bordered w-full max-w-2xl bg-[#ffffff]" required/>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between mt-2">
                            <div class="form-control w-full max-w-sm mr-2">
                                ...
                                <input type="text" name="nama_verifikator" value="{{ $edit[0]->nama_verifikator }}" class="input input-bordered w-full max-w-sm bg-[#ffffff]" required/>
                            </div>
                            <div class="form-control w-full max-w-sm">
